...
SOURCE_SYSTEM = "invenio.bundesarchiv.de"

# Default archive ID for the German Bundesarchiv.
# Must exist in the archive table — as of writing no such archive row
# exists yet (see report). Create one via POST /api/ingest/archives
# before running this scraper against production, then update this
# constant (or pass metadata["archive_id"]) to match.
DEFAULT_ARCHIVE_ID = 10
...
class BackendClient:
    ...
    def _build_headers(cfg):
        headers = {"User-Agent": "scraper-barch/0.1"}
    ...
    def create_record(
        ...
        """Create (or update) a record in the backend. Returns the record_id.
        ...
        body = {
            ...
            "rawSourceMetadata": jsonmod.dumps(
                metadata.get("raw", {}), ensure_ascii=False
            ),
            "lang": cfg.lang,
